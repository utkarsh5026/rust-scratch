...
| Concept | File | Rungs |
|---------|------|-------|
| `Future` trait & `poll` | [`src/bin/future_poll.rs`](./src/bin/future_poll.rs) | 9 — `Poll` enum + `poll(Pin<&mut Self>, cx)` shape (`Ready<T>`) → `.await` IS a poll loop (hand-write `block_on`) → `Pending` = "poll me again" (`YieldOnce`) → delegation combinator `Map<Fut,F>` (forward `cx`) → **state machine by hand** (`enum TwoStep`, one variant per `.await`, carries locals across awaits) → contract violations (waker: no-wake ⇒ parking executor hangs; poll-after-Ready panics) → `async fn` IS a Future (== hand-rolled `TwoStep`; the `!Unpin` wall + `Box::pin`) → `join` by hand (poll both per turn, `Option = None` after Ready = fuse; concurrency ≠ parallelism) → capstone: desugar an `async fn` (2 awaits + runtime branch) into `enum BranchMachine`, drive both via a real flag-waker parking executor, prove equal over `0..20` |
| `Pin` & `Unpin` | [`src/bin/pin_unpin.rs`](./src/bin/pin_unpin.rs) | 9 — construct `Pin<Box<T>>`/`Pin<&mut T>`, read through the pin → `Unpin` = pinning is a no-op (safe `get_mut` escapes freely) → two doors: safe `get_mut` (Unpin) vs unsafe `get_unchecked_mut` (SAFETY: mutate-in-place, never move out) → **THE WHY**: inline self-ref struct, move it ⇒ self-pointer goes STALE (deref-after-move = UB) → `PhantomPinned` ⇒ `!Unpin` ⇒ `Pin::new` refuses (E0277), `Box::pin` is the door → a `!Unpin` pin has no safe `&mut` ⇒ no `mem::swap`; only unsafe in-place edits → `Future::poll(self: Pin<&mut Self>)` by hand, driven via `Box::pin`+noop-waker `block_on` → pin projection by hand: structural `map_unchecked_mut`→`Pin<&mut Child>` (`!Unpin` field) vs non-structural `&mut` (`Unpin` field) = what `pin-project` generates → capstone: a SOUND self-referential struct (`PhantomPinned` + `Box::pin` + wire `ptr = &value` AFTER pinning; moving out won't compile, E0507; Miri-clean) |
| tokio essentials | [`src/bin/tokio_essentials.rs`](./src/bin/tokio_essentials.rs) | 9 — `#[tokio::main]` + `.await` a sleep → `spawn`→`JoinHandle`, spawn-all-then-await = concurrency → `join!` concurrent on one task (tuple, no spawn) → `select!` races, losers dropped mid-flight (cancellation) → `'static`+`Send` wall; `JoinHandle`→`Result<_,JoinError>` catches panics → **blocking the executor** starves other tasks (`yield_now` vs `thread::sleep`) → `spawn_blocking` + `mpsc` pipeline (backpressure, EOF on last-sender drop) → `interval`/`timeout` + `select!` loop w/ `biased;` shutdown → capstone: concurrent worker pool (`Arc<Mutex<mpsc::Receiver>>` queue + `spawn_blocking` + results mpsc; lock only around `recv`) |
